...
bool ValidDateFormat(const char* dateFormat, const char* monthFormat, const char* yearFormat) const;

void init(const char* dateFormat, const char* monthFormat, const char* yearFormat);
char* getDateFormat();
char* getMonthFormat();
char* getYearFormat();
...
DateFormat const& operator=(DateFormat const& d);
...
void init(Day d, Month m, Year y)
throw(invalid_argument, domain_error, out_of_range);

bool OutOfRange(int year) const;
bool ValidDomain(int day, int month, int year) const;

friend inline Date nextDate(const Date& D);
friend inline Date prevDate(const Date& D);

int dayofweek(int d, int m, int y) const;
void setstrdate(const char* c)
throw(invalid_argument, domain_error, out_of_range);
